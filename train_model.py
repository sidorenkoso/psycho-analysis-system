...
# Нове правило: Ризик (1), якщо бали > 18 АБО якщо бали > 15, але людина дуже довго думає (більше 2.5 сек)
y = np.where((np.sum(X[:, :5], axis=1) > 18) | ((np.sum(X[:, :5], axis=1) > 15) & (X[:, 5] > 2.5)), 1, 0)
